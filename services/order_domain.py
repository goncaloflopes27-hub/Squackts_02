...
def editor_allowed_states(current_state: str | None, *, is_create: bool) -> list[str]:
    if is_create:
        return initial_creation_states_ordered()
    state = (current_state or "rascunho").strip().lower()
    allowed = {state, *STATE_TRANSITIONS.get(state, set())}
    return [candidate for candidate in ORDERED_STATES if candidate in allowed]
