...
class OrderService:
    ...
    def create_order(self, payload: dict[str, object]) -> int:
        ...
        with transaction(self.conn):
            validated["numero"] = self._next_order_number(now)
    ...
    def _normalize_production_type(self, raw_value: str) -> str:
    ...
    def _process_items(self, order_id: int, raw_items: list[dict[str, int]], order_state: str, now: str) -> tuple[list[dict[str, object]], int]:
        ...
        for raw in raw_items:
            ...
            ptype = self._normalize_production_type(str(product["tipo_producao"]))
    ...
    def _next_order_number(self, now: str) -> str:
        year = now[:4]
        key = f"orders.sequence.{year}"
        seq_settings = self.settings.get_int(key, default=0)
        seq_db = self.orders.get_max_sequence_for_year(year)
        seq = max(seq_settings, seq_db) + 1
        self.settings.set_int(key, seq)
