...
from services.order_domain import ORDERED_STATES, initial_creation_states_ordered
...
class OrderEditorDialog(QDialog):
    def __init__(
        ...
        if order:
            status_options = list(ORDERED_STATES)
        else:
            status_options = initial_creation_states_ordered()
        ...
        self.status.input.currentTextChanged.connect(lambda _v: self._validate_form())
    ...
    def _save(self) -> None:
        ...
        QTimer.singleShot(320, lambda: animate_dialog_close(self, super().accept))
