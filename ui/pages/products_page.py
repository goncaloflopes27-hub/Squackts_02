...
class ProductsPage(QWidget):
    def __init__(self, service: ProductService) -> None:
        ...
        self.table.setHorizontalHeaderLabels(["Produto", "SKU", "Produção", "Preço", "Custo", "Margem"])
    ...
    def refresh(self, select_product_id: int | None = None) -> None:
        ...
        for i, row in enumerate(self.products):
            ...
            values = [
                ...
                format_currency(float(row["custo"])),
                f"{margin:.1f}%",
            ]
    ...
    def _filtered_products(self) -> list[Row]:
        ...
        if not self.search_text:
            return rows
        return [row for row in rows if self.search_text in str(row["nome"]).lower() or self.search_text in str(row["sku"]).lower()]
    ...
    def _export_csv(self) -> None:
        ...
        try:
            with csv_path.open("w", newline="", encoding="utf-8") as handle:
                ...
                writer.writerow(["id", "sku", "nome", "tipo_producao", "preco", "custo", "stock", "stock_minimo", "ativo"])
    ...
    def _render_detail(self, product: Row | None) -> None:
        ...
        self.detail.add_content_widget(PanelHeader(str(product["nome"]), str(product["sku"]), PlaceholderThumbnail("POD")))
