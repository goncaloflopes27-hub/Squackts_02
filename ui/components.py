...
class KeyValueCard(AppCard):
    def __init__(self, title: str, entries: list[tuple[str, str]]) -> None:
        ...
        for key, value in entries:
            layout.addWidget(InfoRow(key, value))
...
class InfoRow(QWidget):
    def __init__(self, key: str, value: str) -> None:
        ...
        right = QLabel(value)
        right.setStyleSheet(f"font-weight:650;")
...
class Toolbar(QWidget):
    def __init__(self, *widgets: QWidget) -> None:
        super().__init__()
        self.setStyleSheet(
            f"background:{COLORS.surface}; border:1px solid {COLORS.border}; border-radius:{RADIUS.lg}px;"
            f"QWidget:hover {{border:1px solid {COLORS.border_strong};}}"
        )
